...
def predict_custom_image(img_path):
    from tensorflow.keras.preprocessing impor image
    import numpy as np
    ...
    if prediction[0] < 0.5:
        print(f"Это кот(точность: {prediction[0]})")
    else:
        print(f"Это собака (точность: {prediction[0]})")

predict_custom_image("images.jpeg")
...
def cat_dog(photo):
    try:
        ...
        if not os.path.exists(model_path):
            url = "https://drive.google.com/uc?export=download&id=1Hwmo74ealoJRdzeLB_zH3qC34xpXb5Ta"
